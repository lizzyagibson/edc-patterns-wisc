...
# M_AGE + M_EDU + MARITAL_STATUS + HOME_SCORE + M_IQ + ALCOHOL + SMOKER_IN_HOME + mat_hard
as.data.frame(model_sum)[,-c(9:10)] %>% 
  rownames_to_column(var="beta") %>% 
  mutate(beta = c("alpha", "age", "edu", "marital", "home", "iq", "alcohol", "smoker", "mat_hard",
...
N = nrow(bayes)
C = ncol(bayes) - 6 - 1
x = model.matrix(WISC ~ M_AGE + M_EDU + MARITAL_STATUS + HOME_SCORE + M_IQ + ALCOHOL + SMOKER_IN_HOME + mat_hard, 
                 data = bayes)[,-1]
...
load("./Stan/pattern2_infp2nonc_fit.rda")
load("./Stan/pattern1_infp1nonc_fit.rda")
```
...
mcmc_areas(p2_non,
  ...
  "beta_int" = "Interaction", 
  "beta_sex" =  "Sex female",
  "beta_p" = "P2 in males")) + xlim(c(-14,14))
```
...
mcmc_areas(p2_inf,
  ...
  "beta_int" = "Interaction", 
  "beta_sex" =  "Sex female",
  "beta_p" = "P1 in males")) + xlim(c(-14,14))
```
