...
traceplot(p1_non, pars = trace, inc_warmup = TRUE)
color_scheme_set("mix-blue-pink")
mcmc_trace(p1_non, pars = trace, n_warmup = 1000,
                 facet_args = list(nrow = 3, labeller = label_parsed))

traced <- stan_trace(p1_non, pars = trace, nrow = 3)
traced
...
p1non_coef = cbind(ext_p1$beta_int, ext_p1$beta_sex, ext_p1$beta_p) %>% 
  as_tibble() %>% 
  rename(int=1, sex=2, Males=3) %>% 
             mutate(Females = Males + int) %>% 
  dplyr::select(-sex, -int)
