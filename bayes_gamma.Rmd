...
bayes_un = bind_cols(ppp_cov, e_wa_un, var_wa_un, alpha_w, beta_w) %>% 
...
x = model.matrix(WISC ~ M_AGE + M_EDU + MARITAL_STATUS + HOME_SCORE + M_IQ + ALCOHOL + mat_hard, 
                 data = bayes_un)[,-1]
sex = model.matrix(WISC ~ SEX, data = bayes_un)[,-1]
y = bayes_un$WISC
...
# Normal
P1_un = bayes_un$P1
sd_ewa_un = bayes_un %>% dplyr::select(varP2:varP1) %>% mutate_all(~sqrt(.))
sdP1_un = sd_ewa_un$varP1

# Gamma
Walpha1_un = bayes_un$walpha1
Wbeta1_un  = bayes_un$wbeta1
...
save(p1_gamma_un, file = "./Stan/fits/pattern1_noninf_fit.rda")
...
p1_norm_un <- stan(
  file = "./Stan/models/bn2mf_score_reg_int.stan",  # Stan program
    ...
    iter = 5000,                  # total number of iterations per chain
    cores = 2)                    # number of cores
save(p1_norm_un_dat, file = "./Stan/fits/pattern2_inf_fit.rda")
```
