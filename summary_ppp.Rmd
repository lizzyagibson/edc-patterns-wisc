...
add_ci4interaction <- function(fit, term1, term2) {
    ...
    # this is st error
    #ste.Pf  <- std.Pf/sqrt(length(resid(fit))) # SAMPLE SIZE
...
int_sum = summary(fit, c("beta_int", "beta_sex", "beta_p"))$summary[,-c(9:10)]
...
# this is std error
# ste_f  <- sqrt(abs(var_f))/sqrt(length(resid(fit))) # SAMPLE SIZE

lci.f <- mean(beta_f) - 1.96*se_f
uci.f <- mean(beta_f) + 1.96*se_f

# std error
stderror_f = se_f/sqrt(9612.772)
...
new_var = tibble(beta = "P in females", mean = round(mean(beta_f), 4), 
                 se_mean = round(stderror_f,4), sd = round(se_f,4)) %>% 
          bind_cols(., q)

as.data.frame(int_sum) %>% rownames_to_column(var="beta") %>% 
  bind_rows(.,new_var) %>% 
