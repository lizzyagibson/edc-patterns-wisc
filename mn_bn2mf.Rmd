...
edc <- 
  mn_edc %>% 
  dplyr::select(sid, grep("sg_adj", colnames(.))) %>% 
  drop_na() %>% 
  clean_names() %>% dplyr::select(-sid)
edc

names(edc) <- str_to_upper(str_remove(names(edc), "_sg_adj"))
...
cormat <- get_lower_tri(round(cor(edc_re, use = "complete.obs", method = c("spearman")),2))
