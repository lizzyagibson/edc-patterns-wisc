...
source(here::here("../BN2MF/Results/compare_functions.R"))
source(here::here("../BN2MF/Results/fig_set.R"))
```
...
```{r}
edc = read_csv("./Data/ppp_dat.csv") %>% dplyr::select(1:9, 11:13, 10, 14:17)

edc_std <- apply(edc, 2, function(x) x/sd(x)) %>% as_tibble()

# corr does not care if you standardize
ggc <- ggcorr(edc, c("everything", "spearman"),
       size = 0,  # set this to zero
       layout.exp = 3) + 
       theme_minimal()

# Create labels and plot
lbs = str_pad(str_replace(str_to_upper(names(edc)), "_", "-"), 7)
cor_names <- tibble(x = seq(edc), y = seq(edc), 
                  lbs = lbs)
...
cormat <- get_lower_tri(round(cor(edc, use = "complete.obs", method = c("spearman")),2))

as_tibble(cormat) %>% pivot_longer(MEHHP:BPA) %>% drop_na() %>% filter(value != 1) %>% 
  pull(value) %>% sort()
...
```{r, fig.width=18}
#pdf("./Figures/mn2_edc_loadings.pdf", width = 18)
applied %>% 
