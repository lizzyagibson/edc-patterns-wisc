...
```{r}
mn_dat = ppp %>% left_join(., ppp_current, by = "SID") %>% distinct()
ewa <- readMat(here::here("./Data/mn2_EWA.mat"))[[1]] %>% as_tibble() %>% rename(P2 = V1, P1 = V2)

chem_pat = bind_cols(mn_dat, ewa) %>% dplyr::select(c(2:18,44:45))
...
```{r}
chem_pat %>% filter(P2 > 31)
extreme = chem_pat %>% filter(P2 > 31) %>%
  t() %>% as.data.frame() %>% rownames_to_column(var = "Chemical")
```

```{r}
ppp %>% dplyr::select(2:18) %>% 
  pivot_longer(MEHHP:BPA,
               names_to = "Chemical") %>% 
  group_by(Chemical) %>% 
